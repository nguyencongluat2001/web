function JS_PriceTest(baseUrl, module, controller) {
    this.module = module;
 ...
 * @return void
 */
JS_PriceTest.prototype.loadIndex = function () {
    var myClass = this;
    // $('.chzn-select').chosen({ height: '100%', width: '100%' });
    var oForm = 'form#frmPriceTest_index';
...
JS_PriceTest.prototype.loadevent = function (oForm) {
    var myClass = this;
    $('form#frmAdd').find('#btn_create').click(function () {
        myClass.store('form#frmAdd');
    })
 ...
 * @return void
 */
JS_PriceTest.prototype.add = function (oForm) {
 ...
 * @return void
 */
JS_PriceTest.prototype.store = function (oFormCreate) {
    var url = this.urlPath + '/create';
    var myClass = this;
    var formdata = new FormData();
    if ($("#name").val() == '') {
        var nameMessage = 'Tên xét nghiệm không được để trống!';
        var icon = 'warning';
        var color = '#f5ae67';
    ...
    if ($("#code").val() == '') {
        var nameMessage = 'Mã xét nghiệm không được để trống!';
        var icon = 'warning';
        var color = '#f5ae67';
        NclLib.alerMesage(nameMessage,icon,color);
        return false;
    ...
    if ($("#sex").val() == '') {
        var nameMessage = 'Giới tính xét nghiệm không được để trống!';
        var icon = 'warning';
        var color = '#f5ae67';
        NclLib.alerMesage(nameMessage,icon,color);
        return false;
    ...
    formdata.append('code_blood', $("#code_blood").val());
 ...
 * @return void
 */
JS_PriceTest.prototype.createMoneyPackage = function (oFormCreate) {
    var url = this.urlPath + '/createMoneyPackage';
    var myClass = this;
    var data = $(oFormCreate).serialize();
    if ($("#name_bloodtest").val() == '') {
        var nameMessage = 'Tên xét nghiệm không được để trống!';
        var icon = 'warning';
        var color = '#f5ae67';
        NclLib.alerMesage(nameMessage,icon,color);
    ...
    if ($("#address").val() == '') {
        var nameMessage = 'Địa chỉ xét nghiệm không được để trống!';
        var icon = 'warning';
        var color = '#f5ae67';
        NclLib.alerMesage(nameMessage,icon,color);
        return false;
 ...
 * @return void
 */
JS_PriceTest.prototype.loadList = function (oForm, numberPage = 1, perPage = 15) {
    var myClass = this;
 ...
 * @return void
 */
JS_PriceTest.prototype.edit = function (oForm) {
    ...
    if (listitem == '') {
        var nameMessage = 'Bạn chưa chọn xét nghiệm!';
        var icon = 'warning';
        var color = '#f5ae67';
        NclLib.alerMesage(nameMessage,icon,color);
        return false;
    ...
    if (i > 1) {
        var nameMessage = 'Bạn chỉ được chọn một xét nghiệm!';
        var icon = 'warning';
        var color = '#f5ae67';
        NclLib.alerMesage(nameMessage,icon,color);
        return false;
 ...
 * @return void
 */
JS_PriceTest.prototype.editMoneyPackage = function (oForm) {
...
// Xoa mot doi tuong
JS_PriceTest.prototype.delete = function (oForm) {
    var myClass = this;
    var listitem = '';
    var p_chk_obj = $('#table-data').find('input[name="chk_item_id"]');
    $(p_chk_obj).each(function () {
    ...
    Swal.fire({
        title: 'Bạn có chắc chắn xóa vĩnh viễn xét nghiệm này không?',
        icon: 'warning',
 ...
 * @return void
 */
JS_PriceTest.prototype.seeVideo = function (id) {
